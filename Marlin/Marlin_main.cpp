...
inline void gcode_M503() {
  ...
  inline void gcode_M600() {
	  //prevent to extrude when the extruder is cold
...
String setZandEPos(float Epos,float Zpos){
	//set E pos and Z pos
	char* EposCommandM = "";
	EposCommandM = ftostr5rj(Epos);
	int i = 0;
	for (i = 0;i < 5;i++){
		if ((EposCommandM+i)!= ""){
			break;
	...
	char* EposCommand = EposCommandM + i+1;
	String EposCommandStr = EposCommand;
	char* ZposCommandM = "";
	ZposCommandM = ftostr52sign(Zpos);
	i = 1;
	for ( i = 1; i < 5;i ++){
		if ((ZposCommandM+i) != "0"){
			break;
	...
	char* ZposCommand = ZposCommandM+i+1;
	String ZposCommandStr = ZposCommand;
	String strCmdStart = "G92 E";
	String strMiddle = ".50 Z";
	String strCmdEnd = "\n";

	String strSetEPosCommand = strCmdStart + EposCommandStr +strMiddle+ ZposCommand+ strCmdEnd;
	char dCmd[strSetEPosCommand.length()+1];
	strSetEPosCommand.toCharArray(dCmd,strSetEPosCommand.length()+1);
	SERIAL_ECHO("reset E and Z pos command:");
	SERIAL_ECHO(dCmd);
	enqueue_and_echo_command(dCmd);
	SERIAL_ECHO("COMMAN RUN OK!!\n");
	return ZposCommandStr;
}

void heatExtruderAndBed(uint16_t extruderTemp,uint16_t bedTemp){
	//heating extruder and bed
	char heat_extruder_command[10]="";
//	sprintf_P(mess, PSTR("Iteration : %02i"), (int)iterations);
	sprintf_P(heat_extruder_command,PSTR("M109 S%hu\n"),extruderTemp);
	enqueue_and_echo_command(heat_extruder_command);
	char heat_bed_command[10] = "";
	sprintf_P(heat_bed_command,PSTR("M190 S%hu\n"),bedTemp);
	enqueue_and_echo_command(heat_bed_command);
	//command   run ok !!
}
inline void gcode_M9955(){
	SERIAL_ECHO("Run the command of 9959\n");
	SERIAL_ECHO("..............................\n");
	float* XYZPos = settings.GetXDExtruderPos();
	float Xpos = 0;
	float Ypos = 0;
	float Zpos = 0;
	float Epos = 0;
	...
	SERIAL_ECHO(" X: ");
	SERIAL_ECHO(XYZPos[0]);
	Xpos = XYZPos[0];
	SERIAL_ECHO(" Y: ");
	SERIAL_ECHO(XYZPos[1]);
	Ypos = XYZPos[1];
	SERIAL_ECHO(" Z: ");
	SERIAL_ECHO(XYZPos[2]);
	Zpos = XYZPos[2];
	SERIAL_ECHO(" E: ");
	SERIAL_ECHO(XYZPos[3]);
	Epos = XYZPos[3];
	SERIAL_ECHO(" F: ");
	SERIAL_ECHO(XYZPos[4]);
	Fpos = XYZPos[4];
	SERIAL_ECHO("...............................\n");
	SERIAL_ECHO("9959 info output end!!\n");
	long filePos = settings.GetXDFilePos();
	SERIAL_ECHO("Get file POS:");
	SERIAL_ECHO(filePos);
	SERIAL_ECHO("\n");
	uint16_t* temp = settings.GetTemperature();
	SERIAL_ECHO("temprerature......");
	SERIAL_ECHO(temp[0]);
	uint16_t extruderTemp = temp[0];
	SERIAL_ECHO("   ");
	SERIAL_ECHO(temp[1]);
	uint16_t bedTemp = temp[1];
	SERIAL_ECHO("\n");
	char* FileName = settings.GetPrintFileName();
	SERIAL_ECHO("printting file name ....");
...
//	lcd_status_printf_P(0, PSTR("E%i %s"), target_extruder + 1, MSG_HEATING)
	...
	String ZposStr = setZandEPos(Epos,Zpos);//func   using to set the position

	heatExtruderAndBed(extruderTemp,bedTemp);//Func   heating
...
//
//	char* ZposCommand = ZposCommandM+i+1;
////	dtostrf(Zpos,6,2,ZposCommand);
//	strCmdStart = "G92 Z";
//	strCmdEnd = "\n";
//	String strSetZPosCommand = strCmdStart + ZposCommand + strCmdEnd;
//	char cCmd[strSetZPosCommand.length()+1];
//	strSetZPosCommand.toCharArray(cCmd,strSetZPosCommand.length()+1);
//	enqueue_and_echo_command(cCmd);


//	char   c[] = "";
//
//	dtostrf(Epos, 6, 2, c);//将浮点数转成字符串
//
//	strCmdStart="G92 E+";
//	strCmdEnd="\n";
//
//	String strResumeCmd=strCmdStart+c+strCmdEnd;
//
//	char dCmd[strResumeCmd.length()+1];
//	strResumeCmd.toCharArray(dCmd,strResumeCmd.length()+1);
//
//	SERIAL_ECHO(dCmd);
//	enqueue_and_echo_command(dCmd);

//	sprintf_P(set_ZE_pos_command,PSTR("G92 Z%f\n"),Zpos);
	enqueue_and_echo_commands_P(PSTR("G28 X0 Y0\n"));
	String restoreZPos = "G0 Z" + ZposStr + "\n";
	char restoreZCmd[restoreZPos.length()+1];
	restoreZPos.toCharArray(restoreZCmd,restoreZPos.length()+1);
	enqueue_and_echo_command(restoreZCmd);


	char* FposCommandM = "";
	FposCommandM = ftostr5rj(Fpos);
	int i = 0;
	for (i = 0;i < 5;i++){
		if ((FposCommandM+i)!= ""){
			break;
	...
	char* FposCommand = FposCommandM + i + 1;
	String strCmdStart = "G1 F";
	String strCmdEnd = "\n";
	String strSetFPosCommand = strCmdStart + FposCommand + strCmdEnd;
	char fCmd[strSetFPosCommand.length()+1];
	strSetFPosCommand.toCharArray(fCmd,strSetFPosCommand.length()+1);

//	enqueue_and_echo_command(fCmd);



	//restart to print
	char filePosStr[30] = "";
	sprintf_P(filePosStr,PSTR("%ld"),filePos);
	strCmdStart = "M32 S";
	strCmdEnd = "";
	String StrCmdMiddle = "!";
	String continuePrintCommand = strCmdStart + filePosStr + StrCmdMiddle+ FileNameStr;
	char gCmd[continuePrintCommand.length()+1];
	continuePrintCommand.toCharArray(gCmd,continuePrintCommand.length()+1);

	enqueue_and_echo_command(gCmd);
	settings.ResetPowerFlag();
	char hcmd[30]= "M32 S2162190!YUANCE~1.GCO";
//	enqueue_and_echo_command(hcmd);

//	SERIAL_ECHO("runcommand ,the lenght  of gCmd.....\n");
//	SERIAL_ECHO(continuePrintCommand.length()+1);
//	SERIAL_ECHO(gCmd);
//	for (int i = 0;i < 27;i++){
//		if (gCmd[i] == hcmd[i]){
//			SERIAL_ECHO("The same char:");
//			SERIAL_ECHO(gCmd[i]);
//			SERIAL_ECHO("      ");
//			SERIAL_ECHO(i);
//			SERIAL_ECHO("\n");
//		}else{
//			SERIAL_ECHO("The diffrent char,gcmd:");
//			SERIAL_ECHO(gCmd[i]);
//			SERIAL_ECHO("***      hcmd:");
//			SERIAL_ECHO(hcmd[i]);
//			SERIAL_ECHO("num:");
...
void prepare_move_to_destination() {
  ...
  void handle_power_detect() {
    if (!power_detect) {
	  ...
	  if(card.sdprinting){
	  //if(true){
	  char* cpFName=card.printfilename;
	  SERIAL_ECHO(card.printfilename);
	  SERIAL_ECHO("Out put the file name get from card:");
	  SERIAL_ECHO(cpFName);
	  SERIAL_ECHO("\n");
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
   ...
   if (READ(POWER_DETECT_PIN)==POWER_DETECT_INVERTING){
	...
	//when power on,S is LOW锛宲owercount=751�?	//when power off,S is HIGH  powercount=0;
   if(powerCount==0 && power_flag==0 ){
   	handle_power_detect();
    	SERIAL_ECHOLNPGM("Power Down Detected!");
   	power_flag=1;	//power on,cann't detect power and flag==0锟斤拷set flag=1
   }
