...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_current(AxisEnum axis) {
    ...
    void lcd_sdcard_pause() {
    	extruder_lenght = current_position[E_AXIS];
    	enqueue_and_echo_commands_P(PSTR("G0 X5 Y5 F6000\nG91\nG0 Z+5 F1200\nG90\n"));
//    	enqueue_and_echo_commands_P(PSTR("G91\n"));
//    	enqueue_and_echo_commands_P(PSTR("G0 Z+100 F1200\n"));
//    	enqueue_and_echo_commands_P(PSTR("G90\n"));
//    	enquecommand_P(PSTR("G90\n"));
    	SERIAL_ECHO("current E:\n");
    	SERIAL_ECHO(extruder_lenght);
    	SERIAL_ECHO("\n");
    	card.pauseSDPrint();
    ...
    void lcd_sdcard_resume() {
          SERIAL_ECHO("resume E  value:");
          SERIAL_ECHO(extruder_lenght);
          SERIAL_ECHO("\n");

          char   c[] = "";

          dtostrf(extruder_lenght-3.0, 6, 2, c);//将浮点数转成字符串
          ...
          char cCmd[strResumeCmd.length()+1];
          strResumeCmd.toCharArray(cCmd,strResumeCmd.length()+1);

          SERIAL_ECHO(cCmd);
          enqueue_and_echo_command(cCmd);

          SERIAL_ECHO("restart to print\n");
    ...
    void lcd_sdcard_resume() {
    	...
    	SERIAL_ECHO("\n");
    	char cmd[50];
    	sprintf_P(cmd,PSTR("G92 E%s\n"),ftostr71(extruder_lenght - 3));
...
//    	enqueue_and_echo_commands_P(PSTR("G90\n"));
    ...
    //	SERIAL_ECHO("set current position as the absolute position\n");
    //	SERIAL_ECHO(cmd);
...
*/
    ...
    MENU_ITEM(submenu, MSG_CONTROL, lcd_control_menu);
      ...
      if (card.cardOK) {
        if (card.isFileOpen()) {
          ...
          #endif
          MENU_ITEM(gcode, MSG_RELEASE_CARD, PSTR("M22"));
        }
      }
      else {
  ...
  void lcd_cooldown() {
    ...
    void _lcd_level_bed_continue() {
      ...
      #else
        lcd_return_to_status();
        enqueue_and_echo_commands_P(axis_homed[X_AXIS] && axis_homed[Y_AXIS] ? PSTR("G29\nG0 X110 Y110 F9000\nG0 Z0 F300\nG0 Z10 F600\nG0 Z0 F300\n") : PSTR("G28\nG29\nG0 X110 Y110 F9000\nG0 Z0 F300\nG0 Z10 F600\nG0 Z0 F300\n"));
      #endif
    }
...
//	  lcd_goto_screen(lcd_xd_continue_screen);
	  //change according to  3d ymy .CaoBin raiscube 2017/9/27
//		settings.ResetPowerFlag();
	  	enqueue_and_echo_commands_P(PSTR("M9955"));
  ...
  void lcd_prepare_menu() {
    ...
    //
    MENU_BACK(MSG_MAIN);
    //add by buncho,2017.7.1
    //Home Z and move z up
    //
    // Auto Home
    //
    ...
    MENU_ITEM(function, MSG_Z_UP, lcd_move_z_up);
    ...
    #endif
      //Z home
      MENU_ITEM(gcode, MSG_Z_HOME, PSTR("G28 Z0 F100"));
    //
    // Autostart
    //
    #if ENABLED(SDSUPPORT) && ENABLED(MENU_ADDAUTOSTART)
...
//	  MENU_ITEM(function, MSG_SET_HOME_OFFSETS, lcd_set_home_offsets);
   ...
   */
    ...
    #define _MOVE_XY_ALLOWED true
      ...
      //    //move z up,add by buncho 2016.10.20
void lcd_move_z_up()
  {
	if (encoderPosition) {
